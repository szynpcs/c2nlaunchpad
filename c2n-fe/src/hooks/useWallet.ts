...
export function listenToWallet() {
  ...
  useEffect(() => {
    ...
    if (!signer) {
      ...
      const poolAddresses = stakingPoolAddresses.find(v => v.chainId == chain.chainId);
      const { depositTokenAddress, earnedTokenAddress } = poolAddresses || {};
